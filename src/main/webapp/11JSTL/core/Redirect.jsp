...
<body>
<!-- redirec 태그 : 태그가 실행되는 즉시 페이지이동된다. 새페이지에대한 요청이므로 request영역은공유안됨
절대경로 사용시 컨텍스트루트경로는 제외한다

request 영역에 변수저장
페이지 이동시 파라미터 전달. 주소줄에 쿼리스트링으로 표시된다.  -->
...
<!-- response 내장객체 통한 이동시에는 컨텍스트루트 경로를 반드시기술해야한다
또한 파라미터가 한글인 경우 깨짐 현상이 발생하므로 URLEncoder 클래스 통해 인코딩 처리 해줘야한다 -->
<h4>JSP 내장객체 통한 페이지 이동</h4>
<%
